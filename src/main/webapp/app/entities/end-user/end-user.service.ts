...
export class EndUserService {
  ...
  create(endUser: IEndUser): Observable<EntityResponseType> {
    return this.http.post<IEndUser>(this.resourceUrl, endUser, { observe: 'response' });
  }

  update(endUser: IEndUser): Observable<EntityResponseType> {
    return this.http.put<IEndUser>(this.resourceUrl, endUser, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IEndUser>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IEndUser[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
